...
import time
import subprocess
from modern_command_guard import check_command, create_checkpoint
from modern_edit_tools import apply_unified_diff, replace_symbol, validate_python
from modern_symbol_index import render_symbol_context
import re
import logging
...
class ToolExecutor:
    ...
    def execute(self, tool_name: str, arguments: dict) -> str:
        ...
        try:
            if tool_name in {"write_file", "edit_file", "apply_diff", "replace_symbol"}:
                checkpoint = create_checkpoint(self.working_dir, f"checkpoint before {tool_name}")
                if not checkpoint.allowed:
                    reasons = "; ".join(checkpoint.reasons)
                    return f"ERROR: Could not create safety checkpoint before {tool_name}: {reasons}"
            ...
            if tool_name == "run_command":
                ...
                return self._apply_diff(arguments)
    ...
    def _run_command(self, args: dict) -> str:
        ...
        if not guard.allowed:
            ...
            return f"ERROR: Blocked by modern command guard: {reasons}. Command: {command[:120]}"
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if "nothing to commit" in combined and "working tree clean" in combined:
                    result.returncode = 0
                    output += "\nNOTE: git no-op commit treated as success."
    ...
    def _write_file(self, args: dict) -> str:
        ...
        old_content = full_path.read_text() if full_path.exists() else None
        ...
        if lint_result:
            if old_content is None:
                ...
                full_path.write_text(old_content)
            return lint_result + "\nROLLBACK: Restored previous file content after failed lint."
    ...
    def _apply_diff(self, args: dict) -> str:
        path = args.get("path", "")
        ...
        result = apply_unified_diff(old_content, patch)
        if not result.ok or result.new_content is None:
            return f"ERROR: Could not apply diff: {result.message}"

        syntax = validate_python(full_path, result.new_content)
        if not syntax.ok:
            return f"ERROR: Diff rejected: {syntax.message}"

        full_path.write_text(result.new_content)
        return f"OK: Applied diff to {path}"

    def _replace_symbol(self, args: dict) -> str:
        path = args.get("path", "")
        ...
        if not replacement:
            return "ERROR: No replacement provided"
        ...
        old_content = full_path.read_text()
        result = replace_symbol(old_content, symbol, replacement)
        if not result.ok or result.new_content is None:
            return f"ERROR: Could not replace symbol: {result.message}"

        syntax = validate_python(full_path, result.new_content)
        if not syntax.ok:
            return f"ERROR: Symbol replacement rejected: {syntax.message}"

        full_path.write_text(result.new_content)
        return f"OK: Replaced symbol {symbol} in {path}"
...
class LLMClient:
    ...
    def __init__(self, model_config: ModelConfig):
        ...
        self.session = httpx.Client(timeout=3600)
    ...
    def chat(
        ...
        if self.config.provider == "ollama":
            ...
            return self._chat_openai(messages, tools, temperature, response_format)
        else:
    ...
    def chat_structured(
        ...
        try:
            resp = self._post_with_local_readiness_retry(url, payload, timeout=3600)  # v0.9.9c: 600→900 (edit repair on large files)
    ...
    def _chat_openai(
        ...
        """Call OpenAI-compatible endpoint (llama.cpp, vLLM, etc.)."""
        endpoint = (self.config.endpoint or "http://127.0.0.1:8000").rstrip("/")
        url = f"{endpoint}/v1/chat/completions"


        # deepseek format handles thinking separation — no /no_think needed
        ...
        try:
            resp = self._post_with_local_readiness_retry(url, payload, timeout=3600)
            resp.raise_for_status()
            data = resp.json()
        except Exception as e:
        ...
        # If content is empty but reasoning_content exists, model burned tokens on thinking
        # Don't salvage from thinking — it leaks planning notes as responses
        if not content and message.get("reasoning_content"):
            logger.warning("OpenAI response had reasoning_content but no content — returning empty")
            content = ""
    ...
    def _post_with_local_readiness_retry(self, url, payload, **kwargs):
        last_exc = None
        ...
        raise RuntimeError(f"OpenAI-compatible API not ready after retries: {last_exc}")
...
class AgentRunner:
    ...
    def __init__(self, config: Config, working_dir: Path):
        ...
        _pb_path = str(working_dir / "playbook.json")
    ...
    def _run_agent(
        ...
        logger.info(f"  Progress: {agent_config.role} starting with max_rounds={max_rounds}")
        ...
        try:
            while round_count < max_rounds:
                round_count += 1
                logger.debug(f"  Progress: {agent_config.role} round {round_count}/{max_rounds}")
                ...
                if not tool_calls:
                    logger.info(f"  Progress: {agent_config.role} completed after {round_count} rounds")
                ...
                # v1.3: Deduplicate identical tool calls before execution
                deduped_tool_calls = []
                seen_tool_calls = set()
                ...
                for tc in tool_calls:
                    try:
                        ...
                        sig = (
                            fn.get("name", ""),
                            str(fn.get("arguments", ""))[:500]
                        )
                        if sig in seen_tool_calls:
                            logger.warning(
                                f"  Skipping duplicate tool call: {fn.get('name', 'unknown')}"
                            )
                            continue
            ...
            if round_count >= max_rounds:
                ...
                if progress is not None:
                    progress.update("failed", f"hit max rounds {max_rounds}", "agent exceeded tool round budget")
                    logger.warning(f"  Agent progress: {progress.role} {progress.status} — {progress.current_step}")
                else:
                    logger.warning(f"  Agent hit max rounds ({max_rounds})")
                return self._agent_failure_result(
                    output="\n".join(all_output),
                    error=f"Hit max tool rounds ({max_rounds})",
                    failure_type="max_rounds",
            ...
            if (
                "OpenAI-compatible API error" in err_text
                or "401 Unauthorized" in err_text
                or "404 Not Found" in err_text
                or "429 Too Many Requests" in err_text
                or "5" in err_text and "HTTP" in err_text
            ):
    ...
    def _verification_failure_result(
        ...
        *,
        ...
        """Create a standardized failed verification AgentResult."""
        return AgentResult(
            success=False,
            output=output,
            error=error,
    ...
    def _agent_failure_result(
        ...
        *,
        ...
        """Create a standardized failed AgentResult."""
        return AgentResult(
            success=False,
            output=output,
            error=error,
...
"""
    ...
    def run_explore(self, state: TaskState) -> AgentResult:
        ...
        symbol_context = render_symbol_context(self.working_dir, state.goal, limit=90)
        symbol_context = truncate_to_budget(symbol_context, PLAN_PROMPT_BUDGET.get("symbol_context", 12000), "symbols")
        symbol_context = f"\n## Repository Symbol Context\n{symbol_context}\n"
...
DoD rules:
- Only include criteria directly requested by the user.
- Do NOT invent byte-size, file-size, checksum, or extra-content criteria unless explicitly requested.
- For exact text file checks, prefer: `test -f file && grep -qx 'EXPECTED_TEXT' file`
- Keep DoD to the smallest sufficient set.
...
"""
        ...
        if any(kw in goal_lower or kw in file_lower or kw in desc_lower
            ...
            flask_section = FLASK_IMPLEMENTATION_GUIDANCE
...
"""
    ...
    def _run_direct_verification(self, state: TaskState) -> Optional[AgentResult]:
        ...
        source_files = [
            ...
            and (f.name.startswith(active_prefixes) or f.name in {"kb_client.py", "librarian.py", "librarian_store.py"})
        ]
        ...
        if n_total == 0:
            logger.info("  Test suite result: no valid test files found — relying on DoD checks")
        else:
        ...
        for idx, criterion in enumerate(state.dod.criteria):
            ...
            # v0.8.5: Prefer explicit DoD verification commands over proportional test mapping.
            # If the planner gave `verify: command`, execute it and score the criterion directly.
            verify_cmd = getattr(criterion, "verification_command", None)
            if verify_cmd:
                result = self.tool_executor.execute("run_command", {"command": verify_cmd, "timeout": 30})
                output = str(result)
                passed = "EXIT_CODE: 0" in output or output.rstrip().endswith("0")
                criterion.passed = passed
                criterion.evidence = output[:1000]
                test_report[cid] = {
                    "passed": passed,
                    "evidence": criterion.evidence[:500],
                    "command": verify_cmd,
                ...
                if passed:
                    logger.info(f"DoD {cid} PASSED via explicit verify command: {criterion.description[:60]}")
                    all_evidence.append(f"{cid}: PASSED -- `{verify_cmd}`")
                else:
                    logger.warning(f"DoD {cid} FAILED via explicit verify command: {criterion.description[:60]}")
                    all_evidence.append(f"{cid}: FAILED -- `{verify_cmd}`")
                continue
        ...
        return AgentResult(
            ...
            failure_type=None if success else "verification_failed",
    ...
    def _auto_install_dependencies(self, venv_activate_path: Path) -> None:
        ...
        if req_txt.exists() and req_txt.stat().st_size > 0:
            logger.debug("  Auto-installing dependencies from requirements.txt...")
            result = subprocess.run(
