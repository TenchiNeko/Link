...
class IterationResult:
    ...
    test_output: Optional[str] = None  # v0.9.6: For RCA blame detection

    def to_dict(self) -> dict:
        return {
            "success": self.success, "phase": self.phase.value,
            "error": self.error, "failure_type": self.failure_type, "dod_results": self.dod_results,
...
class AgentResult:
    ...
    output: str
    error: Optional[str] = None
    failure_type: Optional[str] = None  # e.g. stuck_loop, max_rounds, timeout, api_error, missing_file
    exit_code: int = 0
...
class AgentTaskProgress:
    """Lightweight progress record for delegated planner/builder work."""
    task_id: str
    role: str
    status: str = "pending"
    current_step: str = ""
    ...
    def update(self, status: str, current_step: str = "", note: str = ""):
        self.status = status
...
class TaskState:
    ...
    job_type: str = JobType.CODE_CHANGE.value  # files modified during build
    ...
    def add_failure(self, result: IterationResult):
        self.failure_history.append({
            ...
            "rca": result.rca,
            "test_output": getattr(result, 'test_output', ''),  # v0.9.6
            "partial_output": (getattr(result, "partial_output", None) or getattr(result, "output", "") or "")[-2000:],
        })
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "edit_feedback": self.edit_feedback,
    ...
    def from_dict(cls, data: dict) -> "TaskState":
        state = cls(
            ...
            blamed_source_files=data.get("blamed_source_files", []),
            edit_feedback=data.get("edit_feedback", []),
            changed_files=data.get("changed_files", []),
            job_type=data.get('job_type', JobType.CODE_CHANGE.value),
        )
        if data.get("dod"):
...
@dataclass
class AdminDecision:
    """Supervisor decision. Metadata/routing only; never direct code execution."""
    action: str
    reason: str
